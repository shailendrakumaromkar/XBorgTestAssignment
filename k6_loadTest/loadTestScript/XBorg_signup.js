...
const signupData = new SharedArray('signup', function () {
  // here you can open files, and then do additional processing or generate the array with data dynamically
  const f = JSON.parse(open('../loadTestData/signup.json'));
...
const randomSignupData = data[Math.floor(Math.random() * signupData.length)];

export default function () {
  let requestURL = "http://localhost:8080/v1/user/signup"
  ...
  const result = http.post(requestURL, JSON.stringify(randomSignupData), params);
